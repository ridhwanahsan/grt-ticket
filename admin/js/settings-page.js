    ...
    $(document).ready(function () {
        ...
        $('#grt-toggle-secret').on('click', function() {
            ...
            if (type === 'password') {
                ...
                $(this).text('Hide'); // i18n handled via text replacement or generic
            } else {
        ...
        $('#grt-test-supabase').on('click', function() {
            ...
            if (!url || !key) {
                $result.css('color', 'red').text('Please enter URL and Secret Key first.');
                return;
            }
            ...
            $.ajax({
                ...
                data: {
                    ...
                    nonce: grtTicketAdmin.settings_nonce,
                    supabase_url: url,
                    supabase_key: key
                },
                ...
                error: function() {
                    $result.css('color', 'red').text('Connection failed (AJAX Error).');
                },
                complete: function() {
                    $btn.prop('disabled', false).text('Test Read');
                }
            });
        });
        ...
        $('#grt-test-supabase-push').on('click', function() {
            ...
            $.ajax({
                ...
                complete: function() {
                    $btn.prop('disabled', false).text('Test Write');
                }
            });
        });

	...
	public function enqueue_scripts() {
		...
		if ( strpos( $screen_id, 'grt-ticket' ) !== false || ( isset( $_GET['page'] ) && strpos( $_GET['page'], 'grt-ticket' ) !== false ) ) {
			...
			$data = array(
				...
				'settings_nonce' => wp_create_nonce( 'grt_ticket_settings_nonce' ),
			...
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Context: Routing logic.
				...
				// Inject form structure
				$form_structure = get_option( 'grt_ticket_form_structure', array() );
				if ( empty( $form_structure ) ) {
					// Default structure
					$default_fields = array(
						array( 'id' => 'category', 'type' => 'select', 'label' => __( 'Category', 'grt-ticket' ), 'required' => true, 'width' => '100', 'is_system' => true ),
						array( 'id' => 'user_name', 'type' => 'text', 'label' => __( 'Your Name', 'grt-ticket' ), 'required' => true, 'width' => '50', 'is_system' => true ),
						array( 'id' => 'user_email', 'type' => 'email', 'label' => __( 'Your Email', 'grt-ticket' ), 'required' => true, 'width' => '50', 'is_system' => true ),
						// Password field removed
						array( 'id' => 'theme_name', 'type' => 'text', 'label' => __( 'Theme / Template Name', 'grt-ticket' ), 'required' => true, 'width' => '50', 'is_system' => true ),
						array( 'id' => 'license_code', 'type' => 'text', 'label' => __( 'License Code', 'grt-ticket' ), 'required' => true, 'width' => '50', 'is_system' => true ),
						array( 'id' => 'title', 'type' => 'text', 'label' => __( 'Issue Title', 'grt-ticket' ), 'required' => true, 'width' => '100', 'is_system' => true ),
						array( 'id' => 'priority', 'type' => 'select', 'label' => __( 'Priority', 'grt-ticket' ), 'required' => true, 'width' => '100', 'is_system' => true ),
						array( 'id' => 'description', 'type' => 'textarea', 'label' => __( 'Describe Your Issue', 'grt-ticket' ), 'required' => true, 'width' => '100', 'is_system' => true ),
					);
					...
					// Ensure user_password is REMOVED
					$new_structure = array();
				...
				wp_add_inline_script( $this->plugin_name . '-form-builder', 'var grtFormStructure = ' . wp_json_encode( $form_structure ) . ';', 'before' );
			}
		}
	}
